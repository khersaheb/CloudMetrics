...
const PricingBlockV1 = ({ planType, type, description, price, users, features, color, buttonText }) => (
   <RctCollapsibleCard customClasses="text-center" colClasses="col-md-6">      
      <h2 className={`text-${color} pricing-title`}><IntlMessages id={type} /></h2>
      <p>{description}</p>
      <Button  color={color} className='btn-block btn-lg'>
         <Link component={Link} to="/app/rize-tracks/techcomputerscienceasliberalart/introductoryprogramming" className="text-white"><IntlMessages id={buttonText} /></Link>
      </Button>
   </RctCollapsibleCard>
);
